(function() {
  const logStyle = 'background:#0066cc;color:#fff;padding:2px 6px;border-radius:3px';
  ...
  function processGifElement(gifElement, popup) {
    ...
    if (gifElement.querySelectorAll(`[class="${className}"]`).length < 1) {
      console.log('%cGiphy Markdown', logStyle, 'process Gif Element '+giphyId);
      gifElement.insertAdjacentElement('afterbegin', markdownLink);
    } else {
      console.log('%cGiphy Markdown', logStyle, 'Gif Element '+giphyId+' already processed');
    }
  }

  function init() {
    console.log('%cGiphy Markdown', logStyle, 'init giphy markdown link');
